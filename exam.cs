    ...
    string[] FilterStrings(string[] inputArray)         // Метод для фильтрации строк по длине
    {
        ...
        {
            if (str.Length <= 3)    // Это условие проверяет длину str. Если длина > 3 символа,то условие истинно.
            {
                resultArray[index++] = str;     // Если условие истинно, то str добавляется в новый массив resultArray на позицию с текущим индексом index, и затем индекс увеличивается на 1
            }
        }
   ...
   int CountStrings(string[] inputArray, Func<string, bool> condition)      // Метод для подсчета строк, удовлетворяющих условию
    {
        ...
        {
            if (condition(str))     // Здесь проверяется условие в классе. Если условие истинно для str, то выполняется код внутри блока 
            {
                count++;    // Если условие истинно, счетчик count увеличивается на 1.
            }
        }
